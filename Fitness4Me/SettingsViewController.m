...
-(void)isValidEmail
{
    ...
    if (emailTextField.text.length>0)
    {
        ...
        if (isValid==NO) {
            [self terminateActivity:@"email is not valid"];
        }
        else {
            ...
            BOOL isReachable =[Fitness4MeUtils isReachable];
            if (isReachable) {
                NSString *requestString =[NSString stringWithFormat: @"%@checkemail=yes&email=%@", UrlPath,emailTextField.text];
...
-(void)doLogIn
{
    ...
    if (isReachable){
        ...
        NSString *requestString =[NSString stringWithFormat:@"%@login=yes&username=%@&password=%@", urlPath,usernameTextField,passwordTextField];
        NSURL *url =[NSURL URLWithString:requestString];
        ASIFormDataRequest   *request = [ASIFormDataRequest   requestWithURL:url];
        [request startSynchronous];
...
-(void)saveUserToDatabase:(NSString*)userLevel
{
    ...
    //    [userDB createDatabase];
    //
    //    User *user = [[User alloc]initWithData:userID:usernameLabel.text:nameTextField.text:userLevel];
    //
    //    [userDB insertUser:user];
    //
    //    [userDB release];
